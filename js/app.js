...
// Wait for Config Loader (Multi-tenancy)
window.addEventListener('lms:configReady', (event) => {
  const clientData = event.detail;

  // Initialize Database with CLIENT's config
  const dbInitialized = LMS.DB.init(clientData);

  // Apply specific client settings if any (e.g. Halls)
  if (clientData.halls) {
    LMS.DEFAULT_HALLS = clientData.halls; // Override default halls
    // Also need to ensure these propagate if first load
  }
...
LMS.App = () => {
  ...
  // Firebase Init handled by loader
...
// ==================== MOUNT handled by initializeApp() ====================
